...
初始化时会将看板复制到你工作区**根目录**（与 `assets/` 同级）：

```bash
./scripts/copy-dashboard-to-workspace.sh
open ./dashboard.html
```

或在 Finder 中双击工作区里的 **`dashboard.html`**。

> 若尚未复制：从 `~/.cursor/skills/fund-lab/dashboard.html` 复制到工作区根目录，或运行上面的脚本。

**重要**：看板里点击 **「授权项目目录」**，选择第 1 步那个含 `assets/` 的文件夹。  
之后 AI 更新 Markdown，在看板点「立即刷新」即可看到最新数据。

**skill 更新看板后**：再运行一次 `copy-dashboard-to-workspace.sh` 覆盖根目录副本。
...
| 知识中枢 · 个人基金知识库 | `assets/fund-knowledge.md`（投资问答沉淀） |
| 知识中枢 · 术语词典 | `skills/fund-lab/references/fund-glossary.md` + `assets/personal-glossary.md` |
...
1. 打开工作区根目录的 **`dashboard.html`**（`/fund-lab 初始化` 时会复制到工作区；或运行 `./scripts/copy-dashboard-to-workspace.sh`）
